...
class InsightFacePreprocessingTests(unittest.TestCase):
    """Verify insightface model preprocessing: scaling, alignment, rotation."""

    def test_raw_rgb_input_has_reference_bbox_margin(self):
        """Confirm insightface input is 96x96 with 1.5x face-box expansion."""
        ...
        # Verify the blob fed to the network matches expected affine warp + channel/batch layout
        scale = 96 / (140 * 1.5)
    ...
    def test_analyze_uses_original_frame_despite_other_crop_rotation(self):
        """Ensure insightface applies no roll correction (uses raw unrotated frame)."""
        ...
        # Roll angle is detected and available, but insightface should not use it
    ...
    def test_roll_correction_levels_both_positive_and_negative_slopes(self):
        """Verify rotation correction aligns tilted eyes to same height after rotation."""
        for rise in (20, -20):
            with self.subTest(rise=rise):
                ...
                # After rotation, both eyes should have nearly identical y-coordinates
